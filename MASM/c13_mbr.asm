         ...
         lgdt [cs: pgdt+0x7c00]             ;gdtr前32位为gdt的线性地址, 后16位gdt的界限
         ...
         ;以下进入保护模式... ...,刷新cs寄存器,指向gdt的2号描述符
         jmp dword 0x0010:flush             ;16位的描述符选择子：32位偏移
                                            ;清流水线并串行化处理器
         [bits 32]               
  flush:                                  
         mov eax,0x0008                     ;加载数据段(0..4GB)选择子,1号gdt描述符
         mov ds,eax
         ...
         ;以下加载系统核心程序 
         mov edi,core_base_address          ;edi->内核程序的物理内存地址
      
         mov eax,core_start_sector          ;eax->内核程序的硬盘逻辑扇号
         mov ebx,edi                        ;ebx = edi = 内核程序的物理内存地址 
         ...
         div ecx                            ; eax/512 -> edx:eax edx->余数 eax->商
 ...
 setup:
         mov esi,[0x7c00+pgdt+0x02]         ;不可以在代码段(保护模式下,代码段只是用来执行的,是否读出取决于属性,但一定不能写入)内寻址pgdt
                                            ;但可以通过4GB的段来访问
         ;建立公用例程段描述符
         ...
         mov [esi+0x28],eax                 ;eax:edx中为64位的段描述符
         mov [esi+0x2c],edx
